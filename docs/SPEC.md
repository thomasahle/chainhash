# ChainHash specification

This defines the function in [`include/chainhash.h`](../include/chainhash.h).
The collision bound is stated in [THEOREM.md](THEOREM.md) and the reasons
for the shape of the function in [DESIGN.md](DESIGN.md). “Block” below
always means a logical 256-byte comb block.
...
The key is exactly 64 independent uniformly random bytes, read as eight
little-endian words, from which the 32 block key words are expanded:
...
Use `chainhash_key_from_bytes(bytes64)`. Zero values, including `s=0`
and `y=0`, are valid; no rejection, division, or inverse is used. The
function itself is defined on the 39 words

```text
kappa[0..31], y, c0, c1, c2, c3, c4, tau
```

and `chainhash_key_from_words(words39)` accepts them directly; the proofs
go through this form (with all 39 words independent, as an internal lemma)
and the tests exercise it. The convenience `chainhash_key_from_seed(seed)`
expands eight successive SplitMix64 outputs into 64 key bytes. It has only
...
`C_t` is a 128-bit polynomial representative; `b_t` is a field word. If a pair's first word is absent, omit the entire
...
as a field word and evaluate the finalizer circuit:
...
have their own instruction cost. It is not a claim that the regular SIMD
loops eliminate every multiplication by zero or one. The executable
[`schedule.c`](../test/schedule.c) checks the identity and count, including
...
field calculation. The full-region kernels use k=4, initialize
...
```c
chainhash_stream stream;
chainhash_init(&stream, &key, 4, 1, chainhash_backend());
chainhash_update(&stream, first, first_len);
chainhash_update(&stream, second, second_len);
uint64_t digest = chainhash_final(&stream);
```
...
`chainhash_partial(&stream)` returns only the message polynomial,
...
is `chainhash_join`. Sum partition block counts, replace a total of zero
...
`chainhash_portable` is serial eager Horner. `chainhash` selects a
backend. `chainhash_evaluate` exposes stride, lazy/eager and backend;
`chainhash_with_backend` exposes the specialized one-shot paths.
`chainhash_selftest()` returns nonzero on success.

Backend IDs are `CH_PORTABLE=0`, `CH_XMM=1`, `CH_YMM=2`,
`CH_ZMM=3`, `CH_NEON=4`. Explicit backend calls require a successful
...
keyed pair products and a running polynomial state. ChainHash's binary
field, comb layout, length placement and finalizer are defined here.
