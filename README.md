...
- **80 random bytes to initialize a key:** the expanded key occupies 328 bytes.
...
int main(void) {
    uint8_t random_bytes[CHAINHASH_RANDOM_BYTES]; /* 80 bytes */
...
The default constructor uses 80 random bytes. The technical documents call this
**model A**: a particular, proved way to derive the internal key values from those
bytes. You do not need to select a model or understand the derivation to use the
default constructor.
...
The blog summarizes the default guarantee as a **61-bit length-adjusted score**.
...
formulas give the sharper bounds above. For the implementation-facing guarantee,
...
[proof project](lean/README.md) establishes both the recommended 80-byte key
setup's bounds and the original construction with 41 independently random
...
Most users should use `chainhash_key_from_bytes()` with 80 random bytes. This
section is for comparing key setups, reproducing older results or inspecting
the precise theorem. All constructors produce a 328-byte key object; reducing
the amount of random input changes the guarantee, not that storage size.
...
- **Original paper setup:** all 41 key words are chosen independently, requiring
  328 random bytes. This gives a different, stronger bound than A.
...
The legacy constructor reproduces the original SMHasher3 seed expansion with
41 successive SplitMix64 values, ordered `k[0..31],u,y,z,c[0..4],tau`. It is
provided for compatibility. It is neither model C nor D, and none of the proved
bounds here certifies that 64-bit seed expansion.
...
<details>
<summary>Migrating from the original header</summary>

The old 328-byte `chainhash_key_from_bytes()` constructor is now
`chainhash_key_from_328_bytes()`. The new default takes 80 random bytes.
The old one-argument `chainhash_key_from_seed()` is now
`chainhash_key_from_splitmix64_legacy()`.

`CHAINHASH_KEY_BYTES` remains **328**, the expanded key's storage size.
`CHAINHASH_RANDOM_BYTES` is **80**, the recommended amount of random input.
Every setup uses the same message-processing algorithm and expanded key layout.
