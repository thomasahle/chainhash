import Std
/-! Independent executable bit-serial reference for ChainHash-128 (B = 512), following
SPEC-128.md's three-level definition and index map. Field: GF(2)[X]/(X^128+X^7+X^2+X+1). -/
...
/-- The key: s, y, c0..c4, tau; kappa[a] = s^(a+1). -/
...
def main (args : List String) : IO Unit := do
  ...
  for line in input.splitOn "\n" do
    ...
    if a[0]! = 1 then key := ChainHash128Vectors.expand key
