...
/-! The complete ChainHash-128 v3 hash: comb PH blocks, level-2 Horner in `y` with the byte
length as leading coefficient, the integer twist modulo `2^128`, and the structured quintic.
Key spaces, SPEC envelopes, and their arithmetic. -/
...
/-- Paper model: `kappa[0..31]`, `y`, `tau`, `c0..c4` as independent uniform field words. -/
abbrev IdealKey := ((Slot → F) × F) × (F × (Fin 5 → F))
/-- Model A: `s`, `y`, `tau`, `c0..c4`; the PH table is `kappa[a] = s^(a+1)`. -/
...
/-- SPEC `d_B(L)`: `1` for `L ≤ 16` (at most 128 bytes), else `min W (2⌈L/32⌉)` with `W = 32`. -/
def degreeBudget (L : ℕ) : ℕ := if L ≤ 16 then 1 else min 32 (2 * ((L + 31) / 32))

/-- SPEC numerators for messages of at most `8L` bytes (`L` counts 64-bit words). -/
def paperNumerator (L : ℕ) : ℕ := blocks (8*L) + 1
/-- SPEC `E_A(L) = p_B(L) + d_B(L)`. -/
def modelANumerator (L : ℕ) : ℕ := blocks (8*L) + degreeBudget L
/-- SPEC coarse all-length model A envelope numerator `p + W`. -/
def coarseNumerator (L : ℕ) : ℕ := blocks (8*L) + 32

def paperEpsilon (L : ℕ) : ℚ≥0 := min 1 ((paperNumerator L : ℚ≥0)/2^128)
def modelAEpsilon (L : ℕ) : ℚ≥0 := min 1 ((modelANumerator L : ℚ≥0)/2^128)
def modelACoarseEpsilon (L : ℕ) : ℚ≥0 := min 1 ((coarseNumerator L : ℚ≥0)/2^128)
...
theorem modelAKey_card : Fintype.card ModelAKey = (2^128)^8 := by
...
theorem paper_envelope_arithmetic (L : ℕ) (hL : 0 < L) : paperNumerator L ≤ 2*L := by
  unfold paperNumerator blocks
  split_ifs <;> omega
-- checkpoint: paper_envelope_arithmetic

theorem modelA_envelope_arithmetic (L : ℕ) (hL : 0 < L) : modelANumerator L ≤ 2*L := by
  unfold modelANumerator degreeBudget blocks
  split_ifs <;> omega
...
theorem modelA_le_coarse (L : ℕ) : modelAEpsilon L ≤ modelACoarseEpsilon L := by
  apply min_le_min_left
  apply div_le_div_of_nonneg_right _ (by positivity)
  exact_mod_cast Nat.add_le_add_left (degreeBudget_le L) _
-- checkpoint: modelA_le_coarse

theorem paper_score_ratio (L : ℕ) (hL : 0 < L) :
    paperEpsilon L ≤ (L : ℚ≥0)/2^127 := by
  have h : (paperNumerator L : ℚ≥0) ≤ 2*L := by
...
theorem modelA_score_ratio (L : ℕ) (hL : 0 < L) :
    modelAEpsilon L ≤ (L : ℚ≥0)/2^127 := by
  have h : (modelANumerator L : ℚ≥0) ≤ 2*L := by
...
theorem coarse_score_ratio (L : ℕ) (hL : 0 < L) :
    modelACoarseEpsilon L ≤ 33*(L : ℚ≥0)/2^128 := by
...
theorem numerators_at_one : paperNumerator 1 = 2 ∧ modelANumerator 1 = 2 ∧ coarseNumerator 1 = 33 := by
  decide
-- checkpoint: numerators_at_one

theorem score_attained : paperEpsilon 1 = 1/2^127 ∧ modelAEpsilon 1 = 1/2^127 ∧
    modelACoarseEpsilon 1 = 33/2^128 := by
  have h2 : ((2 : ℕ) : ℚ≥0)/2^128 = 1/2^127 := by
    rw [div_eq_div_iff (by positivity) (by positivity)]
    norm_num
  refine ⟨?_, ?_, ?_⟩
  · rw [paperEpsilon, numerators_at_one.1, min_eq_right (div_le_one_of_le₀ (by norm_num) (by positivity)), h2]
  · rw [modelAEpsilon, numerators_at_one.2.1, min_eq_right (div_le_one_of_le₀ (by norm_num) (by positivity)), h2]
  · rw [modelACoarseEpsilon, numerators_at_one.2.2, min_eq_right (div_le_one_of_le₀ (by norm_num) (by positivity))]
    norm_num
-- checkpoint: score_attained

theorem paperEpsilon_pos (L : ℕ) : 0 < paperEpsilon L := by
  apply lt_min one_pos
  unfold paperNumerator
  positivity
-- checkpoint: paperEpsilon_pos

theorem modelAEpsilon_pos (L : ℕ) : 0 < modelAEpsilon L := by
  apply lt_min one_pos
  have hp := blocks_pos (8*L)
  unfold modelANumerator
  positivity
-- checkpoint: modelAEpsilon_pos

theorem coarseEpsilon_pos (L : ℕ) : 0 < modelACoarseEpsilon L := by
  apply lt_min one_pos
