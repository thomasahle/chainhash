...
theorem idealKey_collision_equal_length (m m' : Message) (hlen : m.length = m'.length)
    ...
    uniformProb (fun k : IdealKey => hash k m = hash k m') ≤
      ((blocks m.length+1 : ℕ) : ℚ≥0)/2^64 := by
...
theorem lengthField_ne_zero {n : ℕ} (hn : n < 2^64) (hpos : 0 < n) : lengthField n ≠ 0 := by
  intro h
  have he := lengthField_injective hn (show 0 < 2^64 by norm_num) (h.trans lengthField_zero.symm)
  omega
...
theorem family_collision_unequal_length {A : Type*} [Fintype A] [Nonempty A]
    ...
    uniformProb (fun k : (A × F) × (F × (Fin 5 → F)) =>
      ...
      ((max (blocks m.length) (blocks m'.length)+1 : ℕ) : ℚ≥0)/2^64 := by
  ...
  have he : ((max (blocks m.length) (blocks m'.length) : ℕ) : ℚ≥0)/Fintype.card F +
      1/Fintype.card F = ((max (blocks m.length) (blocks m'.length)+1 : ℕ) : ℚ≥0)/2^64 := by
    rw [field_card fieldRepr]
    push_cast
    ring
  exact h.trans_eq he
