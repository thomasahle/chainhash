...
def hashBytes (k : Key39) (m : List UInt8) : UInt64 :=
  digestWord (hash (decodeIdeal k) (bytesToMessage m))

def modelAHashBytes (k : Fin 64 → Byte) (m : List UInt8) : UInt64 :=
...
theorem modelA_collision_bound_bytes (L : ℕ) (hL : 0 < L) (hcap : 8*L < 2^64)
    (m m' : List UInt8) (hm : m.length ≤ 8*L) (hm' : m'.length ≤ 8*L) (hne : m ≠ m') :
    uniformProb (fun k : Fin 64 → Byte => modelAHashBytes k m = modelAHashBytes k m') ≤ modelAEpsilon L := by
