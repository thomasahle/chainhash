...
/-! ChainHash-128 v3 (B = 512): the comb layout of `chainhash128_v3.h` / SPEC.md.
...
/-- Model A key exponent: `kappa[2C] = s^(2C+1)` for the first word, `kappa[2C+1] = s^(2C+2)`. -/
def exponent (j : Slot) : ℕ := 2 * j.1.val + (if j.2 then 2 else 1)
/-- SPEC index map `i = 8W*R + 16C + 8h + j` with `W = 32` and zero-based block `t = 8R + j`. -/
...
/-- SPEC `p(ell)`: `p(0) = 1`; `ell = 4096Q + r` gives `8Q` if `r = 0`, else `8Q + min 8 ⌈r/16⌉`. -/
def blocks (n : ℕ) : ℕ :=
...
/-- SPEC examples: lengths 1,17,33,113,256,2048 have 1,2,3,8,8,8 blocks; 2049 has 8 for B=512. -/
