...
/-! The key: the PH table is `kappa[a] = s^(a+1)`. Equal-length differences are polynomials
in `s` whose partner exponents are bounded by `d_B(L)`; for at most 128 bytes every
...
theorem seeded_coefficients_collision (L : ℕ) (hL : 0 < L) (m m' : Message)
  ...
  have hsupport (j : Slot) (hj : data m t j ≠ data m' t j) : 2 * wordIndex t j < L := by
    by_contra h
    have hz := wordAt_zero m (wordIndex t j) (by omega)
    have hz' := wordAt_zero m' (wordIndex t j) (by omega)
  ...
  have hb : uniformProb (fun s : F => ph (active m t) (data m t) s =
    ...
    · apply ph_frobenius_bound (active m t) (data m t) (data m' t) (degreeBudget L) ?_ ⟨j,hj,hne⟩ ?_
      · intro i
        ...
        constructor
        · simp only [data, wordAt_zero m (wordIndex t (i,true)) (by omega), map_zero]
        · simp only [data, wordAt_zero m' (wordIndex t (i,true)) (by omega), map_zero]
      · intro i _ hi
...
theorem collision_equal_length (L : ℕ) (hL : 0 < L) (m m' : Message)
    (hm : m.length ≤ 8*L) (hlen : m.length = m'.length) (hne : m ≠ m') :
    uniformProb (fun k : Key => chainHash k m = chainHash k m') ≤ epsilon L := by
  have hc : (Fintype.card F : ℚ≥0) = (2 : ℚ≥0)^128 := by
    rw [field_card fieldRepr]
    norm_cast
  have h' : uniformProb (fun k : Key => chainHash k m = chainHash k m') ≤
      ((degreeBudget L+blocks m.length : ℕ) : ℚ≥0)/2^128 := by
    unfold chainHash expandKey hash coefficients
    rw [← hlen, ← hc]
...
-- checkpoint: collision_equal_length

/-- The certificate `min 1 ((p_B(L)+d_B(L))/2^128)` for at most `8L` bytes. -/
theorem collision_bound_message (L : ℕ) (hL : 0 < L) (hcap : 8*L < 2^128)
  ...
  · exact collision_equal_length L hL m m' hm hl hne
  · have h := family_collision_unequal_length powerKey m m'
    ...
    exact div_le_div_of_nonneg_right (by exact_mod_cast (show
      ...
      (by positivity)
-- checkpoint: collision_bound_message

/-- The coarse all-length envelope `min 1 ((p+W)/2^128)`. -/
theorem coarse_collision_bound_message (L : ℕ) (hL : 0 < L) (hcap : 8*L < 2^128)
    (m m' : Message) (hm : m.length ≤ 8*L) (hm' : m'.length ≤ 8*L) (hne : m ≠ m') :
    uniformProb (fun k : Key => chainHash k m = chainHash k m') ≤
      coarseEpsilon L :=
  (collision_bound_message L hL hcap m m' hm hm' hne).trans (epsilon_le_coarse L)
-- checkpoint: coarse_collision_bound_message

/-- Every partner exponent is at most `W = 32`: the all-length seeded degree. -/
...
/-- Block form of the coarse envelope: `p` bounds both block counts,
...
-- checkpoint: coarse_collision_bound_message_blocks
