...
/-- `gfmul`: the 128-bit remainder of the unreduced binary-polynomial product. -/
def wordMul (a b : Word 128) : Word 128 := lowWord ((pack 128 a * pack 128 b) %ₘ modulus)

/-- Unsigned addition modulo 2^128, including carries. -/
def wordAdd (a b : Word 128) : Word 128 :=
  (wordIntegerEquiv 128).symm (wordIntegerEquiv 128 a + wordIntegerEquiv 128 b)
...
def wordChain5 (c : Fin 5 → Word 128) (v : Word 128) : Word 128 :=
  let y := wordMul v v
...
theorem highWord_add (p q : BitsPolynomial) : highWord (p + q) = highWord p + highWord q := by
  funext i; exact coeff_add p q (i.val + 128)
...
def wordFold (u y : Word 128) (s : List (Word 128 × Word 128)) (p : Word 128) : Word 128 :=
