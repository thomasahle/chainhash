...
theorem splitWords_injective {p q : BitsPolynomial} (hp : p.natDegree < 256)
  ...
  · by_cases hi' : i < 256
    · have he := congrFun hh ⟨i - 128, by omega⟩
      simpa only [highWord, Nat.sub_add_cancel (by omega : 128 ≤ i)] using he
    · have hp' : p.coeff i = 0 := coeff_eq_zero_of_natDegree_lt (by omega)
      have hq' : q.coeff i = 0 := coeff_eq_zero_of_natDegree_lt (by omega)
      rw [hp', hq']

theorem lengthMask_natDegree_le (n : ℕ) : (lengthMask n).natDegree ≤ 255 := by
  have hp := Polynomial.ofFn_natDegree_lt (R := ZMod 2) (by omega : 1 ≤ 128) (lengthWord n)
  have hq : (1 + X ^ 128 : BitsPolynomial).natDegree ≤ 128 := by
    exact Polynomial.natDegree_add_le_of_degree_le (by simp) (by simp)
  apply Polynomial.natDegree_mul_le.trans
  change (Polynomial.ofFn 128 (lengthWord n)).natDegree + _ ≤ 255
  omega
...
theorem rawDigest_natDegree_lt (m : Message) (k : BlockKey) (t : ℕ) :
    (rawDigest m k t).natDegree < 256 := by
  unfold rawDigest
...
def splitField {F : Type*} [AddGroup F] (repr : Word 128 ≃+ F) (p : BitsPolynomial) : F × F :=
...
def stream {F : Type*} [AddGroup F] (repr : Word 128 ≃+ F) (m : Message) (k : BlockKey) :
