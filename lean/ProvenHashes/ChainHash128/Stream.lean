...
/-- XOR the byte length into both 128-bit halves of the final digest. -/
def lengthMask (n : ℕ) : BitsPolynomial := pack 128 (lengthWord n) * (1 + X ^ 128)

theorem lengthMask_injective {n n' : ℕ} (hn : n < 2 ^ 128) (hn' : n' < 2 ^ 128)
    (h : lengthMask n = lengthMask n') : n = n' := by
  have hm : (1 + X ^ 128 : BitsPolynomial) ≠ 0 := by
    intro hz
    have hc := congrArg (fun p : BitsPolynomial => p.coeff 128) hz
    norm_num [Polynomial.coeff_one] at hc
  exact lengthWord_injective hn hn' (pack_injective 128 (mul_right_cancel₀ hm h))

abbrev BlockKey := Fin 16 × Bool → Word 128
...
theorem rawStream_equal_length_bound (m m' : Message)
    ...
    uniformProb (fun k => rawStream m k = rawStream m' k) ≤ 1 / (2 : ℚ≥0) ^ 128 := by
  classical
