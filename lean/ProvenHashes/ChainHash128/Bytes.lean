...
/-! Ideal-key collision bound for distinct byte messages: equal lengths through the exact
...
-- checkpoint: idealKey_collision_equal_length
...
/-- Ideal-key certificate `min 1 ((p+1)/2^128)` with `p = p_B(L)` for at most `8L` bytes. -/
theorem idealKey_collision_bound_message (L : ℕ) (hL : 8*L < 2^128)
    (m m' : Message) (hm : m.length ≤ 8*L) (hm' : m'.length ≤ 8*L) (hne : m ≠ m') :
    uniformProb (fun k : IdealKey => hash k m = hash k m') ≤ idealKeyEpsilon L := by
  apply le_min (probability_le_one _)
  by_cases hl : m.length = m'.length
  · apply (idealKey_collision_equal_length m m' hl hne).trans
  ...
  · have h := family_collision_unequal_length (fun k : Slot → F => k) m m'
    ...
    exact div_le_div_of_nonneg_right
      ...
      (by positivity)
-- checkpoint: idealKey_collision_bound_message

/-- Block form of the ideal-key certificate: `p` bounds both block counts, `ε ≤ min 1 ((p+1)/2^128)`. -/
...
-- checkpoint: idealKey_collision_bound_message_blocks
