#!/usr/bin/env python3
"""Deterministic corpus; compare the public chainhash128_v3.h (portable, every available
backend x strides 1..8 x eager/lazy x schoolbook/Karatsuba, dispatched one-shot, selftest)
against the independent executable Lean reference vectors/V3_128.lean. Also checks both
against the header's own hard-coded self-test vectors (seed 123; lengths 0, 17, 2049).
Corpus construction and seed 0x4348313238 are the source lane's."""
import pathlib,random,subprocess,hashlib,os
os.environ['LEAN_NUM_THREADS']='32'
root=pathlib.Path(__file__).resolve().parents[1]; out=root/'build'/'v3-128-vectors';out.mkdir(parents=True,exist_ok=True)
...
# Header self-test key: chainhash128_v3_key_from_seed(123) = SplitMix64 bytes, model A.
seed=123; z64=[]
for i in range(16):
...
subprocess.run(prefix+['cc','-O2','-std=c99','-I'+str(root/'include'),str(root/'test/lean_vectors_v3_128.c'),'-o',str(out/'vectors-c')],check=True)
...
with (out/'vectors-lean.out').open('w') as f:subprocess.run(prefix+['lake','env','lean','--run','vectors/V3_128.lean',str(source)],cwd=root/'lean',stdout=f,check=True)
