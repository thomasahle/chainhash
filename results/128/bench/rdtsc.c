...
__attribute__((noinline)) static ch128_word call(const chainhash128_key *k,const void *p,size_t n,int b,int school,int stride,int lazy) {
    return stride?chainhash128_evaluate(k,p,n,stride,lazy,b,school):chainhash128_with_backend(k,p,n,b,school);
}
static uint64_t ticks(void) { _mm_lfence(); return __rdtsc(); }
static double sample(const chainhash128_key *k,const void *p,size_t n,int b,int school,int stride,int lazy,int reps) {
    uint64_t t; int i; ch128_word v; for(i=0;i<16;i++) { v=call(k,p,n,b,school,stride,lazy); sink^=v.lo; }
...
int main(int argc,char **argv) {
    chainhash128_key k=chainhash128_key_from_seed(42); unsigned char *p=(unsigned char *)malloc(262144+64); int b,school,a,i,small=argc>1; (void)argv;
