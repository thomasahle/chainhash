...
static chainhash_v3_key k;
static chainhash_key old;
__attribute__((noinline)) static uint64_t v3(const void *p,size_t n) { return chainhash_v3(&k,p,n); }
...
int main(void) { uint8_t m[320];unsigned i,n,h,r; uint64_t (*fn[])(const void *,size_t)={v3,shipped}; const char *names[]={"chainhash-v3","shipped-256"}; k=chainhash_v3_key_from_seed(123);old=chainhash_key_from_splitmix64_legacy(123);for(i=0;i<320;i++) m[i]=(uint8_t)i;
