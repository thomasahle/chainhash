...
data['meta']={'definition':'ChainHash v3 Horner, comb fixed at four 256-byte blocks per 1 KB region','key_model':'Model A 64 bytes, ideal 39 words','affinity':{'Xeon microbenchmarks':'16-23','Xeon other jobs':'32-95','M2Pro':None},'load_gate':{'M2Pro':{'no_process':'SMHasher3','load1_less_than':4.5,'poll_seconds':60}},'source_sha256':hashlib.sha256((root/'chainhash_v3.h').read_bytes()).hexdigest(),'clock_caveat':'Xeon B/TSC; M2 calibrated estimated cycles, not a privileged hardware cycle counter. Printed GiB/sec assumes 3.5 GHz.'}
gates={}
...
m2=data.get('chainhash-v3',{}).get('M2Pro')
if m2:
...
(root/'speeds_chainhash_v3.json').write_text(json.dumps(data,indent=2)+'\n')
for name,hosts in data.items():
