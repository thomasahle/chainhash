...
#include <stdlib.h>
#include <x86intrin.h>
static chainhash_key key;
static chainhash_x86_key oldx;
static chainhash_key shipped;
__attribute__((noinline)) uint64_t zmm(const void *p,size_t n) { return chainhash_with_backend(&key,p,n,3); }
__attribute__((noinline)) uint64_t xmm(const void *p,size_t n) { return chainhash_with_backend(&key,p,n,1); }
__attribute__((noinline)) uint64_t ymm(const void *p,size_t n) { return chainhash_with_backend(&key,p,n,2); }
...
int main(void) {
    uint8_t *data=(uint8_t *)malloc(262144+64); unsigned i,a,h; const char *names[]={"v3-zmm","x86-shipped","shipped-256","v3-xmm","v3-ymm"}; /* row labels of the archived out/Xeon/rdtsc.csv: v3-* is ChainHash on the ZMM/XMM/YMM path, the other two are the controls */ uint64_t (*f[])(const void *,size_t)={zmm,old,ship,xmm,ymm};
    key=chainhash_key_from_seed(123); oldx=chainhash_x86_key_from_seed(123); shipped=chainhash_key_from_splitmix64_legacy(123);
