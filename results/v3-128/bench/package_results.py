"""Assemble speeds_chainhash128_v3.json from completed raw Speed logs.

Schema: same layout as speeds_chainhash_v3.json (hash -> host -> runs and
...
SOURCES=[
 ('Xeon8375C','Xeon','xeon-provenance.json',['chainhash128-v3.512school','chainhash128-v3.512','chainhash-128','chainhash-v3','XXH3-128','UMASH-128','rapidhash','komihash'],
 ...
 ('Xeon8375C','Xeon-final','xeon-final-provenance.json',['chainhash128-v3'],
  'Public-default binary from the final header; the lane ended after one of two passes'),
 ('M2Pro','M2','m2-final-provenance.json',['chainhash128-v3.512school','chainhash128-v3.512','chainhash-128','chainhash-v3','XXH3-128','UMASH-128','rapidhash','komihash'],
  'Final registrations; public default of this binary still dispatched NEON to Karatsuba (superseded runs recorded under chainhash128-v3)'),
 ('M2Pro','M2-default','m2-default-provenance.json',['chainhash128-v3'],
  'Public default rebuilt after the selection: NEON dispatches to schoolbook'),
]
SUPERSEDED=[('M2Pro','M2','chainhash128-v3','Pre-selection public default (NEON dispatched to Karatsuba); replaced by the M2-default runs, retained as evidence')]
ORDER=['chainhash128-v3.512school','chainhash128-v3.512','chainhash128-v3','chainhash-128','chainhash-v3','XXH3-128','UMASH-128','rapidhash','komihash']
...
for host in ('Xeon8375C','M2Pro'):
    s=get('chainhash128-v3.512school',host,'bulk_bytes_per_cycle');k=get('chainhash128-v3.512',host,'bulk_bytes_per_cycle')
    ...
    selection[host]={'schoolbook_bulk':s,'karatsuba_bulk':k,'schoolbook_over_karatsuba_percent':100*(s/k-1),
                     'schoolbook_small':get('chainhash128-v3.512school',host,'small_cycles'),'karatsuba_small':get('chainhash128-v3.512',host,'small_cycles'),
                     'faster_bulk':'schoolbook' if s>k else 'karatsuba','public_default_bulk':get('chainhash128-v3',host,'bulk_bytes_per_cycle'),
                     'previous_chainhash128_bulk':get('chainhash-128',host,'bulk_bytes_per_cycle'),'xxh3_128_bulk':get('XXH3-128',host,'bulk_bytes_per_cycle')}
selection['chosen']='schoolbook'
selection['rule']='chainhash128_v3() dispatches schoolbook products on XMM, ZMM and NEON; Karatsuba on YMM and portable. Both products give the same digest; the choice only affects speed.'

out['meta']={
 'definition':'ChainHash-128 v3: GCM field GF(2^128), eight interleaved comb blocks of 512 bytes per 4 KiB region, raw 256-bit CLNH accumulators, Horner in an independent y with the byte length leading, integer twist plus quintic finalizer',
 'key_model':'Model A 128 random bytes (s, y, c0..c4, tau); paper model 624 bytes (W+7 words, W=32)',
 'output_bits':128,'default_block_bytes':512,'verification_values':{'512':'0x1FCA728C','256':'0x0F709CAD'},
 'registrations':{'chainhash128-v3':'public entry point chainhash128_v3(): 512-byte blocks, runtime dispatch, product method per the selection rule',
                  'chainhash128-v3.512':'512-byte blocks, Karatsuba bulk products (forced)','chainhash128-v3.512school':'512-byte blocks, schoolbook bulk products (forced)',
                  'chainhash-128':'previous ChainHash-128 (control)','chainhash-v3':'64-bit ChainHash v3 (control)'},
 ...
 'builds':builds,
 'header_sha256':sha(root/'chainhash128_v3.h'),
...
(root/'speeds_chainhash128_v3.json').write_text(text)
for name in ORDER:
    for host in ('Xeon8375C','M2Pro'):
