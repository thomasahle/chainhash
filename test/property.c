...
/* Test-only key of 39 independent words, given as 312 little-endian bytes. */
static chainhash_key ideal_key(const uint8_t b[312]) { uint64_t w[39]; unsigned i; for(i=0;i<39;i++) w[i]=iw(b,312,i); return chainhash_key_from_words(w); }
static uint64_t independent(const uint64_t key[39],const uint8_t *m,size_t n) {
    ...
    for(i=0;i<blocks;i++) v=imul(v,key[32])^c[i];
    free(c);
...
typedef struct { const chainhash_key *key; const uint8_t *m; size_t n; unsigned k; int l,b; uint64_t v,p; } work;
static void *worker(void *vp) { work *w=(work *)vp; chainhash_stream s; if(!w->n) { w->v=w->p=0; return NULL; } chainhash_init(&s,w->key,w->k,w->l,w->b); chainhash_update(&s,w->m,w->n); w->v=chainhash_partial(&s); w->p=s.blocks; return NULL; }
...
int main(int argc,char **argv) {
    ...
    if(!chainhash_selftest()) return 2;
    ...
    { unsigned trial,i,j; for(trial=0;trial<100;trial++) {
        uint8_t bytes[312]; uint64_t w[39],h; chainhash_key kb,kw;
        for(i=0;i<312;i++) bytes[i]=(uint8_t)rnd(&rng);
        for(i=0;i<39;i++) { w[i]=0; for(j=0;j<8;j++) w[i]|=(uint64_t)bytes[8*i+j]<<(8*j); }
        kb=ideal_key(bytes); kw=chainhash_key_from_words(w);
        if(memcmp(&kb,&kw,sizeof(kb))) abort();
        kb=chainhash_key_from_bytes(bytes);
        for(i=0;i<7;i++) w[32+i]=iw(bytes,64,i+1);
        w[0]=iw(bytes,64,0); for(i=1;i<32;i++) w[i]=imul(w[i-1],w[0]);
        kw=chainhash_key_from_words(w); if(memcmp(&kb,&kw,sizeof(kb))) abort();
        h=independent(w,bytes,312); check(chainhash(&kb,bytes,312),h,312,chainhash_backend(),4,1,"byte-key-constructor");
    } }
    for(iteration=0;iteration<count+4120;iteration++) {
        size_t n; uint64_t key[39]; chainhash_key k; uint64_t want; unsigned i,ki; int b,l; uint8_t *m=allocation+(iteration%64);
        ...
        if(iteration%257==0) key[32]=0;
        if(iteration%263==0) key[32]=1;
        if(iteration%269==0) memset(key,0,sizeof(key));
        for(i=0;i<n;i++) m[i]=(uint8_t)rnd(&rng);
        k=chainhash_key_from_words(key); want=independent(key,m,n);
        check(chainhash_portable(&k,m,n),want,n,0,1,0,"reference");
        check(chainhash(&k,m,n),want,n,chainhash_backend(),4,1,"dispatch");
        for(b=0;b<=4;b++) if(chainhash_has_backend(b)) {
            check(chainhash_with_backend(&k,m,n,b),want,n,b,4,1,"specialized");
            for(ki=0;ki<4;ki++) for(l=0;l<2;l++) {
                chainhash_stream st; size_t off=0; unsigned stride=ks[ki];
                check(chainhash_evaluate(&k,m,n,stride,l,b),want,n,b,stride,l,"one-shot");
                chainhash_init(&st,&k,stride,l,b); chainhash_update(&st,NULL,0);
                while(off<n) { size_t take=1+rnd(&rng)%1500; if(take>n-off) take=n-off; chainhash_update(&st,m+off,take); off+=take; }
                check(chainhash_final(&st),want,n,b,stride,l,"stream");
                ...
                { size_t cut=(n/2048)*1024; work a={&k,m,cut,stride,l,b,0,0},z={&k,m+cut,n-cut,stride,l,b,0,0}; uint64_t v,p;
                  concurrent_split(&a,&z); p=a.p+z.p; if(!p) p=1; v=chainhash_join(&k,a.v,z.v,z.p,b)^ch_fmul(n,ch_pow(k.yp[1],p,b),b);
                  check(ch_finish(&k,v,b),want,n,b,stride,l,"split-matrix"); }
            }
        }
        checksum^=want;
