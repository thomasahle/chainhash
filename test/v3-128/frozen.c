...
int main(void) {
    ...
    chainhash128_v3_key key = chainhash128_v3_key_from_seed(123);
#if CHAINHASH128_V3_BLOCK_BYTES != 512
#error "vectors.csv is the 512-byte family"
#endif
    for (i = 0; i < sizeof m; i++) m[i] = (uint8_t)(i * 137 + 29);
    for (i = 0; i < 9; i++) {
        ch128v3_word want = ch128v3_make(lo[i], hi[i]);
        n = lengths[i];
        assert(ch128v3_equal(chainhash128_v3_portable(&key, m, n), want));
        assert(ch128v3_equal(chainhash128_v3(&key, m, n), want));
        for (b = 0; b <= 4; b++) if (chainhash128_v3_has_backend(b)) for (school = 0; school < 2; school++) {
            assert(ch128v3_equal(chainhash128_v3_with_backend(&key, m, n, b, school), want));
            for (stride = 1; stride <= 8; stride++) for (lazy = 0; lazy < 2; lazy++) {
                chainhash128_v3_stream s;
                chainhash128_v3_init(&s, &key, stride, lazy, b, school);
                for (pos = 0; pos < n; pos += 17) {
                    chainhash128_v3_update(&s, m + pos, n - pos < 17 ? n - pos : 17);
                    chainhash128_v3_update(&s, NULL, 0);
                }
                assert(ch128v3_equal(chainhash128_v3_final(&s), want));
            }
        }
    }
