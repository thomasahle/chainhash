...
int main(void) {
 ...
 for(int b=0;b<=4;b++) if(chainhash_has_backend(b)) fprintf(stderr," %d",b);
 ...
 while(scanf("%u %zu",&model,&n)==2) {
  ...
  chainhash_key k;
  if(model) { for(size_t i=0;i<8;i++) for(size_t j=0;j<8;j++) b[8*i+j]=(uint8_t)(w[i]>>(8*j)); k=chainhash_key_from_bytes(b); }
  else k=chainhash_key_from_words(w);
  uint8_t *m=malloc(n+1); for(size_t i=0;i<n;i++){ unsigned x; if(scanf("%u",&x)!=1)return 3; m[i]=(uint8_t)x; }
  uint64_t h=chainhash_portable(&k,m,n);
  for(int backend=0;backend<=4;backend++) if(chainhash_has_backend(backend))
   for(unsigned stride=1;stride<=8;stride++) for(int lazy=0;lazy<=1;lazy++)
    if(chainhash_evaluate(&k,m,n,stride,lazy,backend)!=h) return 4;
  if(chainhash(&k,m,n)!=h)return 5;
  printf("%"PRIu64"\n",h); free(m);
 }
 return 0;
}
