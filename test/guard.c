...
int main(void) {
    ...
    chainhash_key key=chainhash_key_from_seed(876);
    if(base==MAP_FAILED || mprotect(base+page*2,page,PROT_NONE)) return 2;
    memset(base,137,page*2);
    for(n=0;n<=4096;n++) { uint8_t *p=base+2*page-n; uint64_t h=chainhash_portable(&key,p,n); for(b=0;b<=4;b++) if(chainhash_has_backend(b) && chainhash_with_backend(&key,p,n,b)!=h) abort(); }
    if(chainhash(&key,NULL,0)!=chainhash_portable(&key,NULL,0)) abort();
