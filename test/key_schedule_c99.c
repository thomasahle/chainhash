...
int main(void) {
    ...
    assert(CHAINHASH_RANDOM_BYTES==80);
    ...
    /* Check the first and last input words and the complete expanded key. */
    assert(keys[5].words[0]==UINT64_C(0x0706050403020100));
    assert(keys[5].words[40]==UINT64_C(0x4f4e4d4c4b4a4948));
    assert(memcmp(&keys[1],&keys[5],sizeof keys[5])==0);
    for(i=0;i<6;++i)
