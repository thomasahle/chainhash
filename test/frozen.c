...
int main(void) {
    ...
    chainhash_key key = chainhash_key_from_seed(123);
    size_t i, n;
    int b, lazy;
    unsigned stride;
    ...
    for (i = 0; i < sizeof(lengths) / sizeof(lengths[0]); ++i) {
        n = lengths[i];
        if (chainhash_portable(&key, message, n) != digests[i]) return 1;
        for (b = 0; b <= 4; ++b) if (chainhash_has_backend(b)) {
            if (chainhash_with_backend(&key, message, n, b) != digests[i]) return 2;
            for (stride = 1; stride <= 8; ++stride) for (lazy = 0; lazy <= 1; ++lazy) {
                chainhash_stream stream;
                size_t pos = 0;
                chainhash_init(&stream, &key, stride, lazy, b);
                while (pos < n) {
                    size_t take = n - pos < 17 ? n - pos : 17;
                    chainhash_update(&stream, message + pos, take);
                    chainhash_update(&stream, NULL, 0);
                    pos += take;
                }
                if (chainhash_final(&stream) != digests[i]) return 3;
            }
        }
    }
